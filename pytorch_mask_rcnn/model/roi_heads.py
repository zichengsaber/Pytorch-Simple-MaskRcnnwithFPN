...
def fastrcnn_loss(class_logit, box_regression, label, regression_target):
    ...
    # N为roi中bbox的数量(包含正例和反例)，num_pos为bbox为正例的数量 
...
class RoIHeads(nn.Module):
    ...
    def has_mask(self):
        ...
        if self.mask_predictor is None:
            return False
        return True
    # 选择training时的roi和其相应的gt_box.label
    def select_training_samples(self, proposal, target):
        gt_box = target['boxes']
        gt_label = target['labels']
        # 训练的时候proposal 混合了original proposal 和 gt_box
        proposal = torch.cat((proposal, gt_box))
        
        iou = box_iou(gt_box, proposal) #[M,N]
        pos_neg_label, matched_idx = self.proposal_matcher(iou) # match_idx 对应gt_box的index
        pos_idx, neg_idx = self.fg_bg_sampler(pos_neg_label) # 0-N proposal 中正例和反例的index
        idx = torch.cat((pos_idx, neg_idx))
        # match_idx[pos_idx] 正例对应的gt_box
        # [tx,ty,tw,th]
        regression_target = self.box_coder.encode(gt_box[matched_idx[pos_idx]], proposal[pos_idx])
        # 含有正反例的roi
        proposal = proposal[idx]
        # 含有正反例的roi对应的gt_box index
        matched_idx = matched_idx[idx]
        # 含有正反例的roi对应的box—label
        label = gt_label[matched_idx]
        num_pos = pos_idx.shape[0]
        # 将反例的label设置为0 (1+class)
        label[num_pos:] = 0
    ...
    def forward(self, feature, proposal, image_shape, target):
        ...
        # roi_align 获得的feature送入box_predictor
