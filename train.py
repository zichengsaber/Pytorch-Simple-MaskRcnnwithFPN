...
from pytorch_mask_rcnn.datasets.cocoDataset import COCO_Dataset
    
def main(args):
    device = torch.device("cuda" if torch.cuda.is_available() and args.use_cuda else "cpu")
    # ---------------------- prepare data loader ------------------------------- #
    
    train_data=COCO_Dataset(args.data_dir,"train2017",True)
    val_data=COCO_Dataset(args.data_dir,"val2017",True)
    ...
    args.warmup_iters = max(1000, len(train_data))
    # -------------------------------------------------------------------------- #

    print(args)
    num_classes = max(train_loader.dataset.classes) + 1 # including background class
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lr-steps', nargs="+", type=int, default=[15])
    ...
    if args.lr is None:
        args.lr = 0.02 * 1 / 32 # lr should be 'batch_size / 16 * 0.02'
